...
def CommandLineParser(subparsers):
    ...
    group_input.add_argument("--patch-data-dir", dest = "patch_data_dir", default = None,
        help = "Directory of patch data generated by Cellist (not used in the first run of Cellist). ")

    group_model = parser.add_argument_group("Model arguments")
    group_model.add_argument("--cell-diameter", dest = "max_dist", type = int, default = 30,
        help = "Expected cell diameter. Default: 15")
    group_model.add_argument("--spot-imputation-distance", dest = "neigh_dist", type = float, default = 5.0,
        help = "Imputation distance for each spot. Default: 5.0. ")
...
def Update_cell_expr(coord_df_slice, count_df_slice, count_name):
    count_df_slice = pd.merge(count_df_slice, coord_df_slice.loc[:, ('x_y', 'Soft_cell_id', 'Nucleus')], how = "inner", on = 'x_y')
    expr_mat, gene_list, cell_list = get_cell_mat(count_df_seg = count_df_slice, seg_res = 'Soft_cell_id', count_name = count_name)
...
def Assign_nonnucleispot(cell_expr_slice_array, phy_cell_centroid, nonnucl_enhanced_array, nonnucl_phy_coord_df,
    ...
    nonnucl_phy_coord_df.loc[:, 'Soft_cell_id'] = phy_cell_centroid.index[np.argmax(R_norm, axis = 1)]
    nonnucl_phy_coord_df.loc[:, 'Soft_cell_id_prob'] = R_norm.max(axis = 1)
    coord_df_slice.loc[:, 'Soft_cell_id'] = coord_df_slice['Cell_ID'].tolist()
    coord_df_slice.loc[nonnucl_phy_coord_df.index.tolist(), "Soft_cell_id"] = nonnucl_phy_coord_df['Soft_cell_id'].tolist()
    coord_df_slice.loc[:, 'Soft_cell_id_prob'] = 1
    coord_df_slice.loc[nonnucl_phy_coord_df.index.tolist(), "Soft_cell_id_prob"] = nonnucl_phy_coord_df['Soft_cell_id_prob'].tolist()
    coord_df_slice.loc[coord_df_slice['Soft_cell_id_prob'].isna(), "Soft_cell_id"] = np.nan
...
def Soft_assignment(nonnucl_enhanced_array, nonnucl_enhanced_genes, nonnucl_enhanced_spots, 
    ...
    phy_cell_centroid = Update_centroid(phy_coord_df = coord_df_slice_scaled_df.loc[:, ('x', 'y', 'Cell_ID')], cluster_col = 'Cell_ID')
...
def Soft_assignment_iter(nonnucl_enhanced_array, nonnucl_enhanced_genes, nonnucl_enhanced_spots, 
    ...
    for i in range(0, max_iter):
        ...
        coord_df_slice_scaled_df.loc[:,'Soft_cell_id'] = coord_df_slice.loc[:,'Soft_cell_id']
        phy_cell_centroid = Update_centroid(phy_coord_df = coord_df_slice_scaled_df.loc[:, ('x', 'y', 'Soft_cell_id')], cluster_col = 'Soft_cell_id')
...
def Soft_assignment_slice(count_df_patch, coord_df_patch, props_df_patch,
    ...
    xmin_cur, xmax_cur, ymin_cur, ymax_cur, selected_genes, max_dist = 30,
    alpha = 0.8, out_dir = ".", sigma = 1.0, beta = 10, iter_or_not = False, prob_cutoff = 0.7, neigh_dist = 5, epsilon = 1e-4, max_iter = 25):
    ...
    nucl_spot_count = coord_df_slice_nucl['Cell_ID'].value_counts()
    ...
    if coord_df_slice_nucl.shape[0] > 0 and props_df_slice.shape[0] > 0:
        ...
        if props_df_slice.shape[0] > 0:
            nucl_cell_slice = list(set(coord_df_slice_nucl['Cell_ID'].unique()) & set(props_df_slice.index.tolist()))
            nucl_cell_slice = sorted(nucl_cell_slice)
            nucl_cell_rm = set(coord_df_slice_nucl['Cell_ID'].unique()) - set(nucl_cell_slice)
            if nucl_cell_slice:
                coord_df_slice.loc[coord_df_slice['Cell_ID'].isin(nucl_cell_rm), 'Cell_ID'] = np.nan
                ...
                spot_expr_slice_filter_enhanced_array = enhance_spot_expr(coord_df = coord_df_slice.loc[spot_slice_filter, :], 
                    coord_expr_mat = spot_expr_slice_filter, neigh_dist = neigh_dist)
                ...
                coord_df_slice_scaled_df.loc[:, 'Cell_ID'] = coord_df_slice['Cell_ID'].tolist()
                ...
                if type(max_dist) == int:
                    max_dist_slice = max_dist
                elif max_dist == "Distance":
                    median_dist_slice = get_cell_dist(props_df_slice)
                    max_dist_slice = 1.5*median_dist_slice
                else: # nuclei size
                    max_dist_slice = 1.5*props_df_slice['equivalent_diameter_area'].median()
                if max_dist_slice > 40:
                    max_dist_slice = 40
...
def Write_patch(count_df, coord_df, props_df, nucleus_expr_mat, nucleus_gene, nucleus_cell, all_count_mat, all_count_genes, all_count_spots,
    ...
    if coord_df_patch_nucl.shape[0] > 0 and props_df_patch.shape[0] > 0:
        ...
        print('There are %s cells in this patch.' %(len(nucl_cell_patch)))
...
def Soft_assignment_patch(props_file_patch, coord_file_patch, all_spot_count_h5_file_patch, count_file_patch,
    out_dir, num_workers, 
    ...
    # print(props_df_patch)
    # print(coord_df_patch)
    # print(count_df_patch)
    ...
    coord_slice_list = slice_coord(coord_df_patch, span_length, overlap_length)
    print('There are %s slices in the patch.' %len(coord_slice_list))
    res_list = []
    with concurrent.futures.ThreadPoolExecutor(max_workers = 36) as executor:
        for coord_tuple in coord_slice_list:
            ...
            arg_tuple = (count_df_patch, coord_df_patch, props_df_patch, spot_expr_patch_filter, spot_gene_patch_filter, spot_patch, xmin_slice, xmax_slice, ymin_slice, ymax_slice, selected_genes, max_dist, alpha, out_dir, sigma, beta, iter_or_not, prob_cutoff, neigh_dist)
...
def Read_files(props_file, nucleus_count_h5_file, watershed_coord_file, all_spot_count_h5_file, spot_expr_file):
    ...
    coord_df.loc[coord_df['Nucleus'] == 0, "Cell_ID"] = np.nan
...
def Write_tmp_files(count_df_patch, coord_df_patch, props_df_patch, spot_expr_patch_filter, spot_gene_patch_filter, spot_patch, 
    ...
    write_10X_h5(os.path.join(patch_data_dir, "%s_spot_count.h5" %patch_prefix), spot_expr_patch_filter, spot_gene_patch_filter, spot_patch, genome = 'GRCh38', datatype = 'Gene')


def Cellist(props_file, nucleus_count_h5_file, watershed_coord_file, all_spot_count_h5_file, spot_expr_file,
    ...
    if not os.path.exists(out_para):
        os.mkdir(out_para)
    out_dir = out_para
    tmp_dir = os.path.join(out_dir, "tmp")
    ...
    if patch_data_dir:
        ...
        # span_length = 2200
        overlap_length = 100
        coord_patch_list = slice_coord(coord_df, span_length, overlap_length)
        print('There are %s patches to be processed!' %len(coord_patch_list))
        patch_data_dir = "Data_%s" %gene_use
        if not os.path.exists(patch_data_dir):
            os.mkdir(patch_data_dir)
        res_list = []
        with concurrent.futures.ThreadPoolExecutor(max_workers = 36) as executor:
            for i in range(len(coord_patch_list)):
                print('Writing patch %s...' %i)
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers = num_workers) as executor_patch:
        for i in range(len(patch_prefix_list)):
            ...
            print('Processing patch %s...' %patch_prefix)
            ...
            arg_tuple_patch = (props_file_patch, coord_file_patch, all_spot_count_h5_file_patch, count_file_patch,
                out_dir, num_workers, 
                alpha, sigma, beta, gene_use, max_dist, iter_or_not, prob_cutoff, neigh_dist)
    ...
    print('Merging results from patches...')
    ...
    coord_df_merge = coord_df_merge.drop_duplicates(subset = ['x_y', 'Soft_cell_id'])
    coord_df_merge = coord_df_merge.dropna(subset = ['Soft_cell_id'])
    coord_df_merge = coord_df_merge.sort_values(by=['x_y', 'Soft_cell_id_prob'])
    coord_df_merge = coord_df_merge.drop_duplicates(subset = ['x_y'], keep = 'last')
    coord_df_merge = pd.merge(coord_df, coord_df_merge[['x_y', 'Soft_cell_id', 'Soft_cell_id_prob']], how = "left", on = "x_y")
    out_dir = out_para
    print('Writing segmentation results...')
    coord_df_merge.to_csv(os.path.join(out_dir, "%s_segmentation.txt" %(out_para)), sep = "\t", index = False)
    draw_segmentation(coord_df_sub = coord_df_merge, seg_res = "Soft_cell_id", out_prefix = out_para, 
    out_dir = out_dir, x = "x", y = "y", figsize = (80, 80))
    count_df = pd.merge(count_df, coord_df_merge[['x_y', 'Soft_cell_id', 'Nucleus']], how = "left", on = "x_y")
    count_name = count_df.columns[3]
    write_segmentation_h5(count_df, "Soft_cell_id", out_para, out_para, count_name)
    write_segmentation_cell_coord(coord_df_merge,"Soft_cell_id", out_para, out_para)
    print('Writing parameters and basic cell statistics...')
    hvg_list = get_hvg(nucleus_expr_mat, nucleus_gene, nucleus_cell, 1000)
    cell_num = coord_df_merge['Soft_cell_id'].value_counts().shape[0]
    nspot_all = int(coord_df_merge['Soft_cell_id'].count())
    nspot_nucleus = int(coord_df_merge['Cell_ID'].count())
    ...
    try:
        if len(nucleus_cell) > 100000:
            ...
            Cellist_corr_nucl_cyto, Cellist_corr_nucl_cyto_cell = cal_corr_within_seg_nucleus(count_df.loc[count_df['Soft_cell_id'].isin(cells_sample), :], "Soft_cell_id", hvg_list, count_name)
        else:
            Cellist_corr_nucl_cyto, Cellist_corr_nucl_cyto_cell = cal_corr_within_seg_nucleus(count_df, "Soft_cell_id", hvg_list, count_name)
        Cellist_corr_nucl_cyto_df = pd.DataFrame({'Cell': Cellist_corr_nucl_cyto_cell, 'Correlation': Cellist_corr_nucl_cyto})
        Cellist_corr_nucl_cyto_df.to_csv(os.path.join(out_dir, "Cellist_corr_nucl_cyto_df.txt"), index = False, sep = "\t")
        # statics
...
if __name__ == '__main__':
    ...
    Cellist(props_file, nucleus_count_h5_file, watershed_coord_file, all_spot_count_h5_file, spot_expr_file,
